...
成功附加新一期後，腳本會更新 `data/lotto_history_real.csv` 並建立／更新 `data/latest_prediction.json`。JSON 內含前 25 個融合相對權重、各自的 Random Forest／XGBoost 分數、K-Means 群組、5 組組合、奇偶數量、號碼總和與連號對數。若目前最新期數已存在而 JSON 與當前歷史期數及融合模型版本一致，腳本會保持檔案不變，避免排程重跑製造無意義提交。
